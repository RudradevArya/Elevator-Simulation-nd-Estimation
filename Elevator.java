...
class CallComparator implements Comparator<Call> {
    ...
    //Sorting calls based on floor number
    @Override
...
public class Elevator {
    ...
    private List<Call> floorCalls; // stres floorCalls
    private List<Call> carCalls; // stres carCalls
    ...
    private int passengerLoadingTime; // Asuming 1 second
    private int passengerUnloadingTime; // Asuming 1 second
    ...
    //sets carcall  = floor 0 to go to lobby
    ...
    private void checkSequence(Call tempCall) throws InterruptedException {
        ...
        //  looking carCall of the current floorCall
        if (tempCall.getType() == 1 && tempCall.getFloor() == this.currentFloor) {

            int removeIndex = 0;
            boolean foundCarCall = false;

            // search carFloors arr for carCall with the same ID as tempCall
            for (int i = 0; i < this.carCalls.size(); ++i) {
                ...
                if (tempCarCall.getID().equals(tempCall.getID())) {
                    ...
                    // assigning passage to carCall
                    // Same direction and higher than currentFloor  or Opposite direction
        ...
        // looking for all carCalls and floorCalls to can be removed from sequence
